...
function reducer(state, action) {
	for (let i = 0; i < state.length; i++) {
		if (action.data[i] === undefined) {
			if (state[i] !== null)
				state[i] == null;
		} else {
			if (state[i] === null) {
				if (action.data[i] !== null) {
					state[i] = action.data[i];
				}
			} else if (action.data[i] !== null) {
				if (action.data[i]._id !== state[i]._id) {
					state[i] = action.data[i];
				}
			}
		}
...
const SeatingMap = () => {
	const [seats, dispatch] = useReducer(reducer, Array(seatPositions.length).fill(null));
	...
	const seatCirclesNode = useEffect(() => {
		seatCirclesNode = seatPositions.map((seat, index) => {
			return <SeatNode key={index} index={index} name={seat.name} x={seat.x} y={seat.y} active={false} selectSeat={selectSeat} selectedSeat={selectedSeat} user={seats[index]} />;
		});
	});
